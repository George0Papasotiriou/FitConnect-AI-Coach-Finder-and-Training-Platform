...
export async function seed() {
  ...
  for (let i = 0; i < Math.min(traineeIds.length, trainerIds.length); i++) {
    ...
    const messages = [
      { sender: trainerIds[i], content: "Welcome to FitConnect! I'm excited to start training with you. 💪 Let's crush those goals together!" },
  ...
  for (const traineeId of traineeIds) {
    await db.run("INSERT INTO notifications (id, user_id, type, title, body, action_url, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)",
      uuid(), traineeId, 'system', '🎉 Welcome to FitConnect!', 'Start your fitness journey by finding the perfect coach for you.', '/search', '{}');
    await db.run("INSERT INTO notifications (id, user_id, type, title, body, action_url, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)",
