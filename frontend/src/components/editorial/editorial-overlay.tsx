...
import type { ChartCardData } from "@/components/charts/chart-card";
import type { EditorialResponse } from "@/lib/api-types";
...
interface EditorialOverlayProps {
  ...
  onClose: () => void;
  loading?: boolean;
  /** Increment this counter to imperatively trigger the same PDF
   *  download the in-overlay button performs. Voice path uses this. */
...
export function EditorialOverlay({
  ...
  // Voice trigger: parent bumps `downloadRequestId`; we fire the same
  // download flow as the in-overlay button. Guarded on docRef + editorial
  // — graceful no-op if overlay isn't rendered or no editorial is set.
  useEffect(() => {
    if (downloadRequestId === undefined || downloadRequestId === 0) return;
    if (!docRef.current || !editorial) {
      log.warn("voice PDF download skipped — no doc/editorial available");
      return;
    }
    void handleDownload();
    // handleDownload is stable enough (we only depend on downloadRequestId
    // changing). Including it would re-fire on every render.
        ...
        {!showDoc ? (
          ...
          >
            ...
            >
              ...
              <p className="animate-pulse font-mono text-xs tracking-widest text-white/60 uppercase">
