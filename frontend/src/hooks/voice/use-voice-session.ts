...
type StartOptions = {
  ...
  onLogRep?: (exercise: string, reps: number, weight?: number | null) => void;
  onSwitchExercise?: (exercise: string) => void;
      ...
      ws.onmessage = (ev) => {
        ...
        switch (msg.type) {
          ...
          case "action_navigate":
            log.info("action_navigate", { target: msg.target });
            opts.onNavigate?.(msg.target);
            break;
          case "action_click":
            log.info("action_click", { label: msg.label });
            opts.onClick?.(msg.label);
            break;
          case "action_log_rep":
            log.info("action_log_rep", { exercise: msg.exercise, reps: msg.reps, weight: msg.weight });
            opts.onLogRep?.(msg.exercise, msg.reps, msg.weight);
            break;
          case "action_switch_exercise":
            log.info("action_switch_exercise", { exercise: msg.exercise });
            opts.onSwitchExercise?.(msg.exercise);
            break;
          case "interrupted":
